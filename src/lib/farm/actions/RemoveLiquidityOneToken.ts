...
export class RemoveLiquidityOneToken extends BaseAction implements Action {
  ...
  async run(_amountInStep: ethers.BigNumber, _forward: boolean = true): Promise<ActionResult> {
    ...
    if (poolAddr === pools.tricrypto2.address.toLowerCase()) {
      ...
      amountOut = await CurveMetaPool__factory.connect(this._pool, this.sdk.provider).callStatic[
        'calc_withdraw_one_coin(uint256,int128)'
      ](_amountInStep, i, { gasLimit: 10000000 });
    } else if (this._registry === this.sdk.contracts.curve.registries.cryptoFactory.address) {
      amountOut = await CurvePlainPool__factory.connect(this._pool, this.sdk.provider).callStatic.calc_withdraw_one_coin(_amountInStep, i, {
        gasLimit: 10000000,
      });
    }
    ...
    return {
      ...
      encode: (minAmountOut: ethers.BigNumber) =>
        this.sdk.contracts.beanstalk.interface.encodeFunctionData('removeLiquidityOneToken', [
          this._pool,
          this._registry,
          this._tokenOut,
          _amountInStep,
      ...
      decode: (data: string) => this.sdk.contracts.beanstalk.interface.decodeFunctionData('removeLiquidityOneToken', data),
      data: {},
    };
  }
}
