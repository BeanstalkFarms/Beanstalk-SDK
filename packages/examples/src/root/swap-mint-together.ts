...
export async function swap_and_mint(fromToken: Token, amount: TokenValue): Promise<TokenBalance> {
  ...
  // 2. Get swap estimate. This MUST happen before, and separately
  //    from running the farm, since we need an ammount to put into the Permit
  ...
  farm.add(swap.getFarm().steps);
  farm.add(sdk.farm.presets.depositAndMintRoot(account, toToken, permit));
