import { ERC20Token, FarmFromMode, FarmToMode, TokenValue, TokenBalance, TestUtils } from "@beanstalk/sdk";
 ...
 *
 */
export async function paradox_bet(token: ERC20Token, amount: TokenValue): Promise<TokenBalance> {
  // setup
  ...
  farm.add(async () => {
    ...
    return sdk.depot.advancedPipe([
      ...
      sdk.depot.advancedPacket(sdk.contracts.beanstalk, "deposit", [token.address, amountStr, FarmFromMode.EXTERNAL]),
...
(async () => {
  await chain.sendBean(account, sdk.tokens.BEAN.amount(100));
