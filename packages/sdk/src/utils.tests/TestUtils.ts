...
export default class TestUtils {
  ...
  async sendDeposit(
    ...
    const txn = await this.sdk.contracts.beanstalk.connect(await this.provider.getSigner(from)).transferDeposit(
      from,
      to,
      token.address,
      season,
  ...
  async sendBean(
    ...
    console.log(`Sending ${amount.toLocaleString()} BEAN from ${from} -> ${to}...`)
