...
type AdvancedDataTuple = Readonly<[returnDataIndex: number, copyIndex: number, pasteIndex: number]>;
...
export class Pipeline {
   ...
   */
  private static packAdvanced(data: AdvancedDataTuple, preBytes: string = "0x0000") {
   ...
   *
   */
  private static prepareAdvancedData<Type extends AdvancedDataType>(type: Type, copyData: CopyData[Type], value: ethers.BigNumber) {
   ...
   */
  public static encodeAdvancedData(copyData: CopyData[AdvancedDataType], value: ethers.BigNumber = ethers.BigNumber.from(0)) {
    let type: number;
