...
export type AdvancedDataTuple = Readonly<[returnDataIndex: number, copyIndex: number, pasteIndex: number]>;
 ...
 * 
 */
export function prepareAdvancedData<Type extends AdvancedDataType>(
  type: Type,
  ...
  switch (type) {
    ...
    case 1: {
      types.push('bytes32');
      encodeData.push(
        packAdvanced(copyData as AdvancedDataTuple, typeBytes)
      );
      break;
    };
    case 2: {
      ...
      encodeData = encodeData.concat([
        typeBytes,
        (copyData as AdvancedDataTuple[]).map((d) => packAdvanced(d))
      ])
      break;
    }
    default: {
 ...
 */
export function encodeAdvancedData(
  ...
  const { types, encodeData } = prepareAdvancedData(type, copyData, value)
