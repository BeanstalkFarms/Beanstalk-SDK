...
export class Depot {
  ...
  pipe(packet: PipeStruct) {
    return Depot.sdk.contracts.beanstalk.interface.encodeFunctionData(
      'pipe',
      [packet]
    );
  }
  ...
  etherPipe(
    ...
    if (value.eq(0)) {
      console.warn("Optimization: using setherPipe with value = 0 not not recommended. Use sdk.depot.pipe() instead.");
    }
    return Depot.sdk.contracts.beanstalk.interface.encodeFunctionData(
      'etherPipe',
      [packet, value]
  ...
  >(
    ...
    return {
      ...
      data: _contract.interface.encodeFunctionData(
        _method as string,
        _args,
      ),
    } ;
